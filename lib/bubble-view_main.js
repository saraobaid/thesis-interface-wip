...
function save_data(promptKey) {
  // promptKey should look like count_2
  ...
  var item = {
    proli_id,
    prompt: Number(promptKey.substr(6))
  };
  ...
  // before you update log file, you need to load it first
  try {
    load_data(function(log) {
      ...
      log.history.push(item); // add history which proli_id saw which prompt
      log.stats[promptKey]++; // increase count. (ex: count_2)
      console.log(log);
      dbx
        .filesUpload({
          path: log_filename,
...
function generate_main_block(
  ...
  // ex: count_0 = 3, count_1 = 4, count_2 = 3, count_3 = 4
  var minCount = Math.min(stats.count_0, stats.count_1, stats.count_2, stats.count_3); // get the minimum count first. in the above example, minCount is 3
  var minKeys = Object.keys(stats).filter(key => minCount === stats[key]); // get all keys that have same as minimum count. in this example, minKeys are count_0 and count_2
  var randomKey = minKeys[Math.floor(Math.random() * minKeys.length)]; // pick one randomly. ex: count_2
