...
jsPsych.plugins["bubble-view"] = (function () {
  ...
  plugin.info = {
    ...
    parameters: {
      ...
      image_size: {
        ...
        default: [800, 600],
        description:
          "Array specifying the width and height of image presentation in pixels",
      },
      bubble_radius: {
        type: jsPsych.plugins.parameterType.INT,
      ...
      trial_type: {
        ...
        description:
          "The condition to proceed: either fixed number of clicks or fixed duration",
      },
      viewing_duration: {
        type: jsPsych.plugins.parameterType.INT,
  ...
  plugin.trial = function (display_element, trial) {
    ...
    var bubble_history = [];
    if (trial.maximum_click < trial.minimum_click) {
    ...
    function clickLogger(evt) {
      ...
      if (trial.trial_type == "fixed_click_maximum") {
        if (bubble_history.length === trial.minimum_click) {
          document
            .querySelector("#jspsych-bubble-view-submit")
            .insertAdjacentHTML(
              "afterBegin",
              '<button id="jspsych-bubble-view-next" class="jspsych-btn">Next</button>'
            );
          display_element
            .querySelector("#jspsych-bubble-view-next")
    ...
    display_element.innerHTML +=
      '<canvas id="jspsych-bubble-view" width="' +
      trial.image_size[0] +
      '" height="' +
      trial.image_size[1] +
      '"></canvas>';
    var countdownEl = document.createElement("p");
    countdownEl.textContent = "The 'Next' button will appear in 30 seconds.";
    var button = document.createElement("button");
    display_element.appendChild(countdownEl);
    if (trial.trial_type == "fixed_click_maximum") {
      display_element.innerHTML +=
        '<br><span id="jspsych-bubbles-left"></span> bubbles left';
      display_element.innerHTML +=
        '<br><div id="jspsych-bubble-view-submit" style="height:100px;"></div>';
    ...
    if (trial.trial_type == "fixed_viewing_duration") {
      ...
      var x = setInterval(function () {
        countdownEl.textContent = `The 'Next' button will appear in ${timer} seconds.`;
        ...
        if (timer === 29) {
          jsPsych.pluginAPI.setTimeout(function () {
            ...
            countdownEl.parentElement.replaceChild(button, countdownEl);
    ...
    function record_mousepos(e) {
      ...
      var curr_smp = [
        Math.round(performance.now() - trial_onset),
        e.pageX - width_cnt,
        e.pageY - height_cnt,
        0,
      ];
    ...
    function endTrial() {
      display_element.innerHTML = "";
      var image_name = trial.org_image.substring(
        trial.org_image.lastIndexOf("/") + 1
      );
      var trial_data = {
    ...
    function OnClickDrawMask(e) {
      ...
      if ((clickInterval > clickRefractory) & (bubbles_left > 0)) {
        ...
        var newSize = CalcNewImageSize(
          imgBlur.naturalWidth,
          imgBlur.naturalHeight,
          canvas.width,
          canvas.height
        );
